 ...
 * ✅ Confirms that the server can read and write the persistent JSON store.
...
export async function GET() {
  try {
    ...
    // 2️⃣ Read current contents
    const raw = await fs.readFile(DATA_PATH, "utf-8");
    const data = JSON.parse(raw || "{}");

    // 3️⃣ Write a timestamp heartbeat (no data loss)
    data.__lastTest = new Date().toISOString();
    await fs.writeFile(DATA_PATH, JSON.stringify(data, null, 2));

    // 4️⃣ Return diagnostic info
    const stats = await fs.stat(DATA_PATH);
    return NextResponse.json({
      ok: true,
      message: "Persistence check passed ✅",
      file: DATA_PATH,
