...
export async function getChapters(novelSlug: string) {
  const res = await axios.get(`${BASE}/novels/${encodeURIComponent(novelSlug)}/chapters`);
  return res.data;
}

export async function getChapter(novelSlug: string, chapterSlug: string) {
  const res = await axios.get(
    `${BASE}/novels/${encodeURIComponent(novelSlug)}/chapters/${encodeURIComponent(chapterSlug)}`
  );
  return res.data;
}
...
export async function createChapter(novelSlug: string, payload: any) {
  const res = await axios.post(
    `${BASE}/novels/${encodeURIComponent(novelSlug)}/chapters`,
    payload
  );
  return res.data;
}
...
export async function reorderChapters(novelSlug: string, order: string[]) {
  const res = await axios.patch(
    `${BASE}/novels/${encodeURIComponent(novelSlug)}/chapters/reorder`,
    { order }
  );
  return res.data;
}
